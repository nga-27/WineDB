""" Region-related API utility functions for the cmd_app """
...
def search_regions_for_content(name: str) -> List[str]:
    ...
    results = requests.get(f"http://localhost:8282/regions?name={name}", timeout=5)
...
def create_region(name: str, country_id: str | None, description: str | None) -> Tuple[str, bool]:
    ...
    response = requests.post(
        ...
        json={"name": name, "country_id": country_id, "description": description}, timeout=5)
...
def get_country_from_region(region_id: str) -> Tuple[Union[str, None], Union[str, None]]:
    ...
    response = requests.get(f"http://localhost:8282/regions/{region_id}", timeout=5)
    ...
    if country_id is None:
        return None, None
    country_response = requests.get(f"http://localhost:8282/countries/{country_id}", timeout=5)
